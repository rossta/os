  ...
  describe "run" do
    ...
    describe "expectations" do
      before(:each) do
        ...
        @optimist_sim = mock(OptimistSimulator, :simulate! => nil, :algorithm => @optimist)
        OptimistSimulator.stub!(:new).and_return(@optimist_sim)
        @banker       = mock(Banker)
        @banker_sim = mock(BankerSimulator, :simulate! => nil, :algorithm => @banker)
      ...
      it "should simulate banker" do
        ...
        @banker_sim.should_receive(:algorithm).and_return(@optimist)
        simulate_command 1
      end
      it "should simulate optimist" do
        @optimist_sim.should_receive(:simulate!).and_return(nil)
        @optimist_sim.should_receive(:algorithm).and_return(@optimist)
        simulate_command 1
      end
