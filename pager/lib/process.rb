module Paging
  class Process
    ...
    def initialize(a, b, c, io, index = nil)
      ...
      @state          = Paging::ProcessState::Unstarted
    end
    ...
    def start_run
      self.cpu_burst = Paging::OS.random_os(self.max_cpu, self.state) if self.cpu_burst == 0
    ...
    class Running
      def self.cycle(process)
        ...
        if process.remaining_time == 0
          ...
          process.io_burst = Paging::OS.random_os(process.max_io, process.state)
          Blocked
        else
          Running
        end
